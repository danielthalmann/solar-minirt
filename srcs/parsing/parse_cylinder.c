...
/*   Updated: 2022/06/07 18:42:08 by trossel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_error(t_scene *s, int color[3], int n_parsed)
{
	...
	err += parse_check_non_null_vector(&s->shapes->cyl.base[1], ORIEN_ERR, ELEM);
...
{
	b[2] = v3f_cross_product(&(t_v3f){1.0f, 0.0f, 0.0f, 0.0f}, &b[1]);
	b[0] = v3f_cross_product(&b[1], &b[2]);
	v3f_normalize(&b[0]);
	v3f_normalize(&b[1]);
	v3f_normalize(&b[2]);
